...
def linearring_to_polygon(shape):
    for i in range(len(shape)):
        if shape['geometry'][i].geom_type == 'LineString':
            # Create polygon
            polygon = [Polygon(shape['geometry'][i]) for i in range(len(shape))]
        if shape['geometry'][i].geom_type == 'MultiLineString':
            # Create polygon
            geom_ring = [list(shape['geometry'][i].geoms) for i in range(len(shape))]
            polygon = [Polygon(geom_ring[i][0]) for i in range(len(shape))]
...
def create_shoreline_change_points(shoreline, change_polygon):
  ...
  for k in range(len(shoreline['geometry'])):
    ...
    try:
        ...
        line = list(shoreline['geometry'][0].geoms)[0]
